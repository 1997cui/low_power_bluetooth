...
void LCD_Init(void)
{
	...
	GPIO_ConfigurePin(LCD_DC_PORT,LCD_DC,OUTPUT,PID_GPIO,false);
	GPIO_ConfigurePin(LCD_CS_PORT,LCD_CS,OUTPUT,PID_GPIO,false);
...
void LCD_PrintStr_8_16(unsigned char x, unsigned y, unsigned char ch[])
{
    ...
    while (ch[j] != '\0')
    {
        ...
        LCD_Set_Pos(x, y);
        for (i = 0; i < 8; i++)
            LCD_WrDat(font_8_16[c * 16 + i]);
        LCD_Set_Pos(x, y + 1);
        for (i = 0; i < 8; i++)
            LCD_WrDat(font_8_16[c * 16 + i + 8]);
        x += 8;
        j++;
    }
    OS_EXIT_CRITICAL();
