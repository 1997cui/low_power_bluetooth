...
#if (OS_CFG_DYN_TICK_EN == DEF_ENABLED)
    BSP_OS_LP_TickInit();                                       /* Init uC/OS dynamic  time src (LPTimer).              */
#else
...
static  void  BSP_PLL_Init (void)
{
    ...
    while (DEF_BIT_IS_CLR(BSP_REG_MCG_S, BSP_MCG_S_OSCINIT0)) { /* Wait for Crystal Oscillator initialization.          */
        ;
    }
